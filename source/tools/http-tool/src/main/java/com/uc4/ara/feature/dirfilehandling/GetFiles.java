 ...
 * The Class GetFiles.
 */
public class GetFiles extends AbstractPublicFeature {
    ...
    public void initialize() {
        ...
        password = parser.addHelp(parser.addPasswordOption("pwd", "password", false),
                "Optional password if the source machine requires authentication.");

        from = parser.addHelp(parser.addStringOption("src", "source", true),
                "Source file/directory to be downloaded. For HTTP(S), source package must be a file.");

        to = parser.addHelp(parser.addStringOption("tgt", "target", true),
                "Target file/directory of the downloaded file/directory (i.e. C:\\source_dir => /root/target_dir, C:\\source_file.txt => /root/target_file.txt, " +
                "C:\\source_file.txt => /root/target_dir/)");

        preserve = parser.addHelp(parser.addStringOption("prv", "preserve", false),
                "If set to YES, file properties(last modified, read/write permission) would be preserved after being downloaded. Default: NO");

        recursive = parser.addHelp(parser.addStringOption("r", "recursive", false),
                "If set to NO, only direct children of the remote directory would be downloaded. Default: YES");

        overwrite = parser.addHelp(parser.addStringOption("o", "overwrite", false),
                "If set to YES, existing local files would be overwritten by remote files. Default: YES");

        timeout = parser.addHelp(parser.addStringOption("to", "timeout", false),
                "Maximum waiting time for the source machine to be connected. Default: 5000 ms.");
    ...
    public int run(String[] args) throws Exception {
        ...
        try {
            ...
            Logger.log("Cannot get Port value,  use Default Value: 80(http), 443(https).", this.loglevelValue);
        }
        ...
        if (protocolValue.equals("HTTP"))
            ...
            FeatureUtil.logMsg("Unknown Protocol '" + protocolValue + "'. " +
                    "Only support HTTP(S). Aborting ...");
